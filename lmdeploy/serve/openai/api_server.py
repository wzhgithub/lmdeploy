...
async def check_request(request) -> Optional[JSONResponse]:
    ...
    if hasattr(request, 'temperature') and not (2 >= request.temperature >= 0):
        return create_error_response(
            HTTPStatus.BAD_REQUEST,
...
async def chat_completions_v1(request: ChatCompletionRequest,
    ...
    with metrics.E2E_REQUEST_LATENCY.labels(model_name, "chat").time():
        if request.session_id == -1:
            VariableInterface.session_id += 1
            request.session_id = VariableInterface.session_id
        error_check_ret = await check_request(request)
        if error_check_ret is not None:
            metrics.FINISH_REASON_COUNT.labels(model_name, f"{error_check_ret.status_code}").inc()
            return error_check_ret
        if VariableInterface.async_engine.id2step.get(str(request.session_id),
                                                      0) != 0:
            metrics.FINISH_REASON_COUNT.labels(model_name, f"{HTTPStatus.BAD_REQUEST.name}").inc()
            return create_error_response(
                HTTPStatus.BAD_REQUEST,
        ...
        final_res = None
        text = ''
        with metrics.E2E_GENERATE_LATENCY.labels(model_name, "chat").time():
            async for res in result_generator:
        ...
        metrics.GENERATED_TOKENS_COUNT.labels(model_name).inc(final_res.generate_token_len)
        metrics.PROMPT_TOKENS_COUNT.labels(model_name).inc(final_res.input_token_len)
        metrics.FINISH_REASON_COUNT.labels(model_name, final_res.finish_reason).inc()
        return response
